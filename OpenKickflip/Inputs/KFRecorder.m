...
- (void) setupHLSWriterWithEndpoint:(KFS3Stream*)endpoint {
    ...
    if ([OpenKickflip useAudio]) { [_hlsWriter addAudioStreamWithSampleRate:self.audioSampleRate]; }
}
...
{
    return (_readyToRecordVideo && (![OpenKickflip useAudio] || _readyToRecordAudio));
}
...
- (void) startRecording {
    ...
    NSAssert(self.s3Configuration, @"We need an aws s3 configuration set at this point");
    
    KFS3Stream* s3Endpoint = self.s3Configuration;
...
- (void) uploader:(KFHLSUploader *)uploader didUploadSegmentAtURL:(NSURL *)segmentURL uploadSpeed:(double)uploadSpeed numberOfQueuedSegments:(NSUInteger)numberOfQueuedSegments {
    ...
    if ([OpenKickflip useAdaptiveBitrate]) {
        double currentUploadBitrate = uploadSpeed * 1024; // bps
        double minBitrate = [OpenKickflip minBitrate];
        double maxBitrate = [OpenKickflip maxBitrate];
        ...
        double newVideoBitrate = [OpenKickflip useAudio] ? newBitrate - self.aacEncoder.bitrate : newBitrate;
